...
class CausE(Recommender):
    def __init__(self, num_users, num_items,
        ...
        self.user_factors_C = self.rng.normal(loc=0, scale=self.sd_init, size=(self.num_users, self.dim_factor))
    ...
    def train(self, df, iter = 100):
        ...
        if self.metric in ['logloss']:
            while True:
                ...
                u_factor_T = self.user_factors[u, :]
                u_factor_C = self.user_factors_C[u, :]
                i_factor_T = self.item_factors[i, :]
                i_factor_C = self.item_factors_C[i, :]

                if flag_treatment > 0:
                    rating = np.sum(u_factor_T * i_factor_T)
                else:
                    rating = np.sum(u_factor_C * i_factor_C)
                ...
                u_diff_TC = u_factor_T - u_factor_C
                i_diff_TC = i_factor_T - i_factor_C


                if flag_treatment > 0:
                    self.user_factors[u, :] += \
                        self.learn_rate * (coeff * i_factor_T - self.reg_factor * u_factor_T - self.reg_causal * u_diff_TC)
                    self.item_factors[i, :] += \
                        self.learn_rate * (coeff * u_factor_T - self.reg_factor * i_factor_T - self.reg_causal * i_diff_TC)
                    self.user_factors_C[u, :] += \
                        self.learn_rate * (self.reg_causal * u_diff_TC)
                    self.item_factors_C[i, :] += \
                        self.learn_rate * (self.reg_causal * i_diff_TC)
                else:
                    self.user_factors_C[u, :] += \
                        self.learn_rate * (coeff * i_factor_C - self.reg_factor * u_factor_C + self.reg_causal * u_diff_TC)
                    self.item_factors_C[i, :] += \
                        self.learn_rate * (coeff * u_factor_C - self.reg_factor * i_factor_C + self.reg_causal * i_diff_TC)
                    self.user_factors[u, :] += \
                        self.learn_rate * (-self.reg_causal * u_diff_TC)
                    self.item_factors[i, :] += \
                        self.learn_rate * (-self.reg_causal * i_diff_TC)
    ...
    def predict(self, df):
        ...
        for n in np.arange(len(df)):
            ...
            pred_C[n] = np.inner(self.user_factors_C[users[n], :], self.item_factors_C[items[n], :])
            if self.with_bias:
