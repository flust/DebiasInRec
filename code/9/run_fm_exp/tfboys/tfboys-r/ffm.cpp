...
/*
cblas_dgemm( layout, transa, transb, m, n, k, alpha, a, ida, b, idb, beta, c, idc)
...
void ImpProblem::init_params() {
    ...
    fill(HTHZTZ.begin(), HTHZTZ.end(), 0);
    HTHZTZ.resize(d*d, 0);
}
...
void ImpProblem::cg(const shared_ptr<ImpData> &U1, const Vec &G1,
    ...
    ImpDouble g2 = 0, r2, cg_eps = 0.01, alpha = 0, beta = 0, gamma = 0, vHv;

    Vec M(Dfd, 0), R(Dfd, 0), Hv(Dfd, 0), MHZ(Dfd, 0);
    ...
    while (g2*cg_eps < r2 && nr_cg < max_cg) {
        ...
        mm(M.data(), HTHZTZ.data(), MHZ.data(), Df, d, d);

        hs_cross(U1, M, H1, Z1, MHZ, Hv, Hv_);
...
void ImpProblem::solve_block(const shared_ptr<ImpData> &U1, Vec &W1, const Vec &H1, const Vec &Z1, Vec &E1){
    Vec G1(E1.size(), 0);
    Vec S1(E1.size(), 0);
   
    gd_cross(U1, W1, H1, Z1, E1, G1);
    cg(U1, G1, H1, Z1, S1);
...
void ImpProblem::one_epoch() {
    ...
    solve_block(P, Z, W, H, E_p);
    solve_block(U, W, H, Z, E_u);
    solve_block(V, H, W, Z, E_v);
    ...
    for (ImpLong i = 0; i < Uva->m; i++){
        ...
        for (ImpLong j = 0; j < n; j++){
            for(ImpLong k = 0; k < K; k++){
                z[n*k + j] = tensor_product(w_va_p+i*d, hp+j*d, zp+k*d, d);
            }
        }
        ...
        for(YNode* y = Uva->Y[i]; y < Uva->Y[i+1]; y++){
            ImpLong j = y->jdx;
            ImpLong k = y->kdx;
            ImpDouble y_val = y->label;
            label[n*k + j] = y_val;
        }
    }

    Vec Z_all, label_all;
    ImpLong sample_count = Uva->m*n*K; 
    Z_all.reserve(sample_count);
    label_all.reserve(sample_count);
    ...
    for (ImpLong i = 0; i < Uva->m; i++) {
        for (ImpLong j = 0; j < n; j++){
            for(ImpLong k = 0; k < K; k++){
                bool ispos = false;
                for(YNode* y = Uva->Y[i]; y < Uva->Y[i+1]; y++){
                    if( j == y->jdx && k == y->kdx && y->label == 1){
                        ispos = true;
                        break;
                    }
                }

                ImpDouble y = (ispos)? 1.0 : -1.0;
                ImpDouble y_hat = tensor_product(w_va_p+i*d, hp+j*d, zp+k*d, d);
                ImpDouble yy = y * y_hat;
                if (-yy > 0)
                    ploss += (-yy + log1p( exp(yy) ));
                else
                    ploss += log1p( exp(-yy) );
...
void ImpProblem::print_epoch_info(ImpInt t) {
    cout.width(2);
    cout << "iter " << t+1 << " ";
    if (!Uva->file_name.empty() && (t+1) % 3 == 0){
        UTX(Uva->X, mt, E_u, W_va);
        logloss_mn();
        calc_auc_mn();
        cout.width(13);
...
ImpDouble ImpProblem::l_pos_grad(const YNode *y) {
    const ImpDouble y_ij = y->label, y_hat = y->val, expyy = y->expyy;
    ...
    for (ImpLong i = 0; i < U1->m; i++) {
        ...
        for (Node* x = X[i]; x < X[i+1]; x++) {
            ...
            for (ImpInt di = 0; di < d; di++) {
                ...
                assert(G_.size() > jd+id*block_size);
...
void ImpProblem::gd_neg_cross(const shared_ptr<ImpData> &U1, const Vec &W1, const Vec &H1, const Vec &Z1, Vec &G) {
    const vector<Node*> &X = U1->X;

    const ImpLong n1 = H1.size()/d, K1 = Z1.size()/d;

    Vec HTH(d*d, 0), ZTZ(d*d, 0), T(U1->m*d, 0);
    Vec In(n1, 1), Ik(K1, 1), oH(d, 0), oZ(d, 0), oHZ(d, 0);
    ...
    mv(H1.data(), In.data(), oH.data(), n1, d, 0, true);
    mv(Z1.data(), Ik.data(), oZ.data(), K1, d, 0, true);
    hadamard_product(oH.data(), oZ.data(), d, oHZ.data());
    ...
    for (ImpLong i = 0; i < U1->m; i++) {
        ...
        for (Node* x = X[i]; x < X[i+1]; x++) {
            ...
            for (ImpInt di = 0; di < d; di++) {
                const ImpLong jd = idx*d+di;
                G_[jd+id*block_size] += w*(t1[di]-r*oHZ[di])*val;
            }
        }
    }
...
void ImpProblem::hs_neg_cross(const shared_ptr<ImpData> U1, Vec &M,
        const Vec &MHZ, Vec &Hv, Vec &Hv_) {
...
void ImpProblem::line_search(const shared_ptr<ImpData> &U1, const Vec &E, const Vec &H1, 
    ...
    if (w != 0) {
        ...
        Vec Hs(Dfd, 0), MHZ(Dfd*d, 0);
        mm(S1.data(), HTHZTZ.data(), MHZ.data(), Df, d, d);

        fill(Hs_.begin(), Hs_.end(), 0);
        hs_neg_cross(U1, S1, MHZ, Hs, Hs_);
    ...
    if( theta != 1 )
        cerr << "Sub-problem:" << " did line search " << theta << endl << flush;
}
...
ImpDouble ImpProblem::calc_L_pos(const shared_ptr<ImpData> &U1, const ImpDouble theta){
    ...
    for(ImpLong i = 0; i < U1->m; i++){
        for(YNode *y = U1->Y[i]; y != U1->Y[i+1]; y++){
            ...
            const ImpDouble yy = y_hat_new * (ImpDouble) y->label;
...
void ImpProblem::init_L_pos(){
    ...
    for (ImpLong i = 0; i < m; i++) {
        for (YNode* y = U->Y[i]; y < U->Y[i+1]; y++) {
            ...
            if( -yy > 0 )
                res += (-yy + log1p( exp(yy) )) - 0.5 * w * (y->val - r) * (y->val - r);
            else
                res += log1p( exp(-yy) ) - 0.5 * w * (y->val - r) * (y->val - r);
        }
    }
    L_pos = res;
}
