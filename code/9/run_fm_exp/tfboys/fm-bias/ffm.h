...
class ImpProblem {
public:
    ImpProblem(shared_ptr<ImpData> &U, shared_ptr<ImpData> &Uva, shared_ptr<ImpData> &V,
            ImpDouble &lambda, const ImpDouble &w,
            const ImpInt &d, const ImpInt &nr_threads, const ImpInt &nr_pass, const bool is_solve_imp)
        :U(U), Uva(Uva), V(V), lambda(lambda), w(w),
        d(d), nr_threads(nr_threads), nr_pass(nr_pass), is_solve_imp(is_solve_imp){
    ...
    void init();
    void solve();
    void input_whz(const shared_ptr<ImpData> &U1, const Vec &iW1,
...
private:
    ...
    void solve_block(const shared_ptr<ImpData> &U1, Vec &W1, const Vec &H1, const Vec &iW1, const Vec &iH1, Vec &E1);

    void gd_cross(const shared_ptr<ImpData> &U1, const Vec &W1, const Vec &H1,
            const Vec &iW1, const Vec &iH1, const Vec &E1, Vec &G);

    void gd_neg_cross(const shared_ptr<ImpData> &U1, const Vec &W1,
            const Vec &H1, const Vec &iW1, const Vec &iH1, Vec &G);

    void gd_pos_cross(const shared_ptr<ImpData> &U1, const Vec &H1, Vec &G);

    void hs_cross(const shared_ptr<ImpData> &U1, Vec &M,const Vec &H1, Vec &Hv, Vec &Hv_);
    ...
    void cg(const shared_ptr<ImpData> &U1, const Vec &G1, const Vec &H1, Vec &S1);
    void line_search(const shared_ptr<ImpData> &U1, const Vec &E, const Vec &H1, const Vec &G, Vec &S1);

    void calc_delta_y_cross(const shared_ptr<ImpData> &U1, const Vec &XS, const Vec &H1);
